...
async def handle_whatsapp_cloud_webhook(webhook_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Handle WhatsApp Business API webhook for media messages with cloud storage
            ...
            if change.get("field") == "messages":
                ...
                for message in messages:
                    ...
                    if message.get("type") in ["image", "video", "document"]:
                        ...
                        # Get media URL (requires WhatsApp Business API token)
                        media_url = f"https://graph.facebook.com/v18.0/{media_id}"
                        
                        # Process the media with cloud storage
                        agent = CloudWhatsAppMediaAgent()
                        result = await agent.process_whatsapp_media(
                            media_url=media_url,
